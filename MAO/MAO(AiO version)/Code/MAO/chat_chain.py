...
def _unique_run_id() -> str:
    """
    Return a short identifier that is unique even when multiple runs start
    within the same wall-clock second.  Combines timestamp + process PID so
    that parallel conda subprocesses — which always have distinct PIDs — can
    never collide on directory names.
    """
    ts  = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
    pid = os.getpid()
    return f"{ts}_pid{pid}"
...
class ChatChain:
    ...
    def execute_step(self, phase_item: dict):
        ...
        if phase_type == "SimplePhase":
            ...
            if phase in self.phases:
                if (phase == 'ProcessReview' and self.chat_env.env_dict['continue_sign_review'] == "0") \
                        or (phase == 'FormatReview' and self.chat_env.env_dict['continue_sign_test'] == "0"):
                    pass
                else:
            ...
            multiplicityExitNum = phase_item.get('multiplicityExitNum')
            ...
            if (phase == 'Review' and self.chat_env.env_dict['continue_sign_review'] == "0") \
                    or (phase == 'Test' and self.chat_env.env_dict['continue_sign_test'] == "0"):
                pass
            else:
    ...
    def get_logfilepath(self):
        ...
        # Use _unique_run_id() so that parallel runs starting in the same
        # second (common with max_workers > 1) get distinct log paths.
        ...
        log_filepath = os.path.join(
            directory,
            "{}.log".format("_".join([self.project_name, self.org_name, run_id])),
        )
        self.start_time = start_time
        self._run_id = run_id   # store for consistent use in pre_processing
        return log_filepath
    ...
    def pre_processing(self):
        if self.chat_env.config.clear_structure:
            ...
            for filename in os.listdir(directory):
                ...
                # Only remove plain files; never remove .py, .log, or directories.
                # Guard with existence check — parallel runs may have already removed it.
                if os.path.isfile(file_path) \
                    ...
                    try:
                        ...
                        # Another parallel process already removed it — fine.
                        pass

        software_path = os.path.join(
            directory,
            # Use the same _run_id that was used for the log file, so both
            # paths are always consistent and unique per process.
            "_".join([self.project_name, self.org_name, self._run_id]),
        )
    ...
    def post_processing(self):
        ...
        if os.path.exists(process_txt_path):
            try:
                ...
                process_no_dummy_bpmn_path = os.path.join(
                    self.chat_env.env_dict["directory"], "process_no_dummy.bpmn"
                )
                subprocess.run(
                    ["python",
                     os.path.join(helper_dir, "remove_dummy.py"),
                     final_bpmn,
                     process_no_dummy_bpmn_path],
                    check=True,
                )
                log_and_print_online(
                    f"Dummy tasks removed. Output saved to: {process_no_dummy_bpmn_path}"
                )
        ...
        if self.chat_env_config.git_management:
            ...
            os.system("cd {} && echo version >> v{:.1f}".format(
            ...
            os.system('cd {} && git commit -m "v{} Final Version"'.format(
                self.chat_env.env_dict["directory"], self.chat_env.codes.version))
            ...
            git_info += (completed_process.stdout if completed_process.returncode == 0
        ...
        # ── Move log file into the software directory ──────────────────────
        # FIX: the log filepath is overwritten in run.py to point INSIDE the
        # software directory already (chat_chain.log_filepath = .../log.txt).
        # If src == dst (already in place) we skip the move silently.
        # Any other OSError (e.g. file not found due to a parallel race) is
        # caught and logged — it must never crash the whole run, because the
        # BPMN output has already been written successfully.
        filepath = os.path.dirname(__file__)
        root = os.path.dirname(filepath)
        ...
        src = self.log_filepath

        # Resolve to absolute paths before comparing to catch same-file with
        # different representations (relative vs absolute, symlinks, etc.)
        try:
            src_abs = os.path.realpath(os.path.abspath(src))
            dst_abs = os.path.realpath(os.path.abspath(dst))
        except Exception:
            src_abs, dst_abs = src, dst

        if src_abs == dst_abs:
            # Log is already in the correct location — nothing to do.
            pass
        else:
            try:
                ...
                # The log file was already moved or was never created
                # (e.g. parallel run cleaned WareHouse between creation and
                # move).  Not a fatal error — the BPMN output is safe.
                print(f"[post_processing] Warning: log file not found at {src!r}; "
                      f"skipping move (likely already in place or cleaned up).")
            except shutil.Error as e:
                # shutil.Error includes "same file" errors on some platforms.
                print(f"[post_processing] Warning: log move skipped — {e}")
            except OSError as e:
                print(f"[post_processing] Warning: could not move log file — {e}")

    def self_task_improve(self, task_prompt):
        self_task_improve_prompt = (
            'I will give you a short description of a software design requirement, '
            'please rewrite it into a detailed prompt that can make large language model '
            'know how to make this software better based this prompt, '
            'the prompt should ensure LLMs build a software that can be run correctly, '
            'which is the most import part you need to consider. '
            'remember that the revised prompt should not contain more than 200 words, '
            'here is the short description:"{}". '
            'If the revised prompt is revised_version_of_the_description, '
            'then you should return a message in a format like '
            '"<INFO> revised_version_of_the_description", '
            'do not return messages in other formats.'
        ).format(task_prompt)

        role_play_session = RolePlaying(
            assistant_role_name="Prompt Engineer",
            assistant_role_prompt=(
                "You are an professional prompt engineer that can improve user input "
                "prompt to make LLM better understand these prompts."
            ),
        ...
        log_and_print_online(
            ...
            "**Improved Task Prompt**: {}".format(task_prompt, revised_task_prompt)
        )
