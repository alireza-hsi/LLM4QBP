...
def get_initial_messages(txt_path: str):
    ...
    Returns a list of one dict suitable for ChatCompletions.
    """
    with open(txt_path, "r", encoding="utf-8") as f:
        qbp_txt = f.read()

    # (long prompt omitted here for brevity; use your existing prompt with {qbp_txt})
    prompt = f"""
    Your role: you are an expert in clinical pathway generation and process modeling, familiar with common process constructs such as exclusive choice, do-redo loops, and partial orders. Your task is to analyze the textual description of a clinical guideline and transform it into a process model in the POWL language. When generating a model, be as precise as possible and capture all details of the process in the model. Also act as the process owner and use your expertise and familiarity with the process context to fill in any missing knowledge.
    ...
    Provide the Python code that recursively generate a POWL model. Save the final model in the variable 'final_model'. Do not try to execute the code, just return it. Assume the class ModelGenerator is properly implemented and can be imported using the import statement: from utils.model_generation import ModelGenerator. ModelGenerator provides the functions described below:
    - activity(label) generates an activity. It takes 1 string arguments, which is the label of the activity.
    - xor(*args) takes n >= 2 arguments, which are the submodels. Use it to model an exclusive choice structures, i.e., if you have several possible paths where only one of them can be taken (either or), then use xor to combine them. If a decision is made based on some condition at some point in a process, you should model an exclusive choice between the two paths starting after this decision xor(path_1 path_2) where path_1 and path_2 are subprocess that encapsulates the full sequence of actions following each decision. You can use xor(submodel, None) to make a submodel optional; i.e., to model an exclusive choice between executing this submodel or skipping it.
    ...
    Note 1: Each Quality-based procedure (clinical practice guideline) has a section named "Implementation Consideration" at the end. They can be relevant to the process (e.g., Predischarge planning should commence shortly after admission to hospital.) or irrelevant to the process (e.g., At a system level, OHTAC end-of-life recommendations should be fully implemented.). Only add the relevant ones to the process model and ignore irrelevant considerations.
    Note 2: This clinical practice guideline offers a set of recommendations that are not strictly sequential—some must be followed in order, while others can be implemented independently.

    This is the process description: {qbp_txt}
    """
    return [{"role": "user", "content": prompt}]


def generate_code_from_messages(messages, openai_api_key=None, model="GPT_4o1"):
    """
    Given a full ChatCompletion-style message list, call the LLM and extract the
    Python code block. Returns the raw code (no ```python fences).
    """
